...
const runTestCases_js_1 = require("./codeExecutor/runTestCases.js");
const questions_js_1 = require("./db/questions.js");
const socket_io_1 = require("socket.io");
const roomsHelper_js_1 = require("./utils/roomsHelper.js");
// define constants
...
const setupSocketIO = (httpServer) => {
    ...
    io.on(CONNECTION_SOCKET_EVENT, (socket) => {
        socket.on(CODE_SUBMISSION_SOCKET_EVENT, async (code, questionId, language) => {
            const result = await (0, runTestCases_js_1.runTestCases)(code, questionId, language);
        ...
        socket.on(SEND_ROOMS_SOCKET_EVENT, () => {
            socket.emit(GET_ROOMS_SOCKET_EVENT, (0, roomsHelper_js_1.publicRooms)(rooms));
        });
        socket.on(DISCONNECT_SOCKET_EVENT, () => {
            ...
            io.emit(GET_ROOMS_SOCKET_EVENT, (0, roomsHelper_js_1.publicRooms)(rooms));
        });
        ...
        socket.on(CREATE_ROOM_SOCKET_EVENT, (isPublic) => {
            const roomCode = (0, roomsHelper_js_1.roomCodeGenerator)().toString();
            ...
            if (rooms.has(roomCode)) {
                ...
                if (room && room.players[0] !== socket.id) {
                    ...
                    if (room.players.length == PLAYERS_PER_ROOM) {
                        ...
                        const question = questions_js_1.questions[5];
